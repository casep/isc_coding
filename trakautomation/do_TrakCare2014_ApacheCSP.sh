...
trakapacheconf_SLES() {
	...
	CONFPRT=$CONFDIR/t2014-EPS.conf
#	echo $CONF
}
trakapacheconf_RHEL() {
...
# on with the show
echo "Trak 2014 Apache & CSP config for $SITE : $ENV ($INST: $TRAKNS)"
...
if [ -n "$PRT" ]; then
	...
	cp conffiles/apache-t2014report.conf $CONFPRT
	chmod 644 $CONFPRT
	...
	# install Trak config
	cp conffiles/apache-t2014.conf $CONF
	chmod 644 $CONF
	# apply custom settings
