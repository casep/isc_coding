...
echo "########################################"
INST=`instname $SITE $ENV ANALYTICS$VER`
TRAKNS=`traknamespace $SITE $ENV`
TRAKPATH=`trakpath $SITE $ENV ANALYTICS$VER`
...
cp expect/TrakVanillaT2016_Install_install.expect $TMPDIR/trakextract
chmod 755 $TMPDIR/trakextract/TrakVanillaT2016_Install_install.expect
olddir=`pwd`
cd $TMPDIR/trakextract
${olddir}/expect/TrakVanillaT2016_Install_unzip.expect $installer
chown $CACHEUSR.$CACHEGRP $TMPDIR/trakextract -R

$TMPDIR/trakextract/TrakVanillaT2016_Install_install.expect $INST $TMPDIR/trakextract $ENV $TRAKNS ${TRAKPATH} /trakcare
cd ${olddir}
...
./expect/TrakVanillaT2016_Install_cleanup.expect $INST $TRAKNS $SITE_UC ${TRAKPATH}/web/custom/$SITE_UC/cdl
