...
check_Unix() {
	...
	perforcedir=`trakpath $SITE $ENV DB$VER`/integration/perforce/
	...
	errors=0
	# check all ownership
	badownership=`find "$perforcedir" ! -name lost+found ! -user $CACHEUSR | wc -l`
	...
	badgroup=`find "$perforcedir" ! -name lost+found ! -group $CACHEGRP | wc -l`
	...
	baddirperms=`find "$perforcedir" ! -name lost+found -type d ! -perm -2770 | wc -l`
	...
	badfileperms=`find "$perforcedir" -type f ! -perm -440 | wc -l`
...
preflightargs $@
# get on with the job
echo "*CHECK - Integration Perforce/CCR Filesystem Permissions"
