...
check_Unix() {
	...
	badfileperms=`find "$webdir" -type f -perm +111 | wc -l | sed 's/ //g'`
	if [ $badfileperms -gt 0 ]; then
		errors=$(($errors+$badfileperms))
		echo "=ALERT - $badfileperms files found in \"$webdir\" with Execute bits set (run: find \"$webdir\" -type f -perm +111)"
	fi
	# ok if no errors
	if [ $errors -eq 0 ]; then
